<script>
    // vercel library for handling openai api chat logic
			...
			<div class="h-full grid grid-rows-[1fr_auto] gap-1">
				<section class="w-full max-h-[400px] p-4 overflow-y-auto space-y-4">
					<div class="grid grid-cols-[auto_1fr] gap-2">
						<Avatar src="https://i.pravatar.cc/?img=Sthenos" width="w-12" />
						<div class="card p-4 variant-soft rounded-tl-none space-y-2">
							<header class="flex justify-between items-center">
								<p class="font-bold">Sthenos</p>
								<!-- <small class="opacity-50">{bubble.timestamp}</small> -->
							</header>
							<p> Talk to me for personalized nutrition and exercise plans.
								You'll get meal plans, workout plans, routines, and
								motivation to help you create a healthier lifestyle that you love.
								Let's get started on your journey to a healthier you!</p>
						</div>
					</div>
					{#each $messages as message, i}
						...
						{:else}
							...
							<div class="grid grid-cols-[1fr_auto] gap-2">
								...
								<Avatar src="https://i.pravatar.cc/?img=friend" width="w-12" />
							</div>
						{/if}
					{/each}
				</section>
				<form on:submit={handleSubmit} class="input-group input-group-divider grid-cols-[auto_1fr_auto]">
					<div class="input-group-shim"> 🗣️</div>
					<textarea  
						bind:value={$input}
						class="textarea bg-transparent border-0 ring-0"
						name="prompt"
						id="prompt"
						placeholder="Write a message..."
						rows="1"/>
					
					<button type="submit" class="button variant-filled-primary">Send</button>
				</form>
